...
def main(cfg: DictConfig):
    ...
    for test_dataset_name in evaluator.test_dataloader_dict.keys():
        ...
        with open(os.path.join(save_path, f"results-{test_dataset_name}-zeroshot.json"), "w") as outfile:
            json.dump(evals, outfile)

        zeroshot_binary = {
            k: {
                _k: _v
                for _k, _v in v["zeroshot_binary"].items()
        ...
        with open(os.path.join(save_path, f"results-{test_dataset_name}-zeroshot.txt"), "w") as outfile:
            outfile.write(st)
